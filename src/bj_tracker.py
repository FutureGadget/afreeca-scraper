...
def is_same_date_in_kst(new_datetime: datetime):
    ...
    if not new_datetime:
        ...
        now = get_now_as_kst()
        return (
            now.year == new_datetime.year
            and now.month == new_datetime.month
            and now.day == new_datetime.day
        )
...
class ShineeTracker:
    ...
    def had_live_today(self):
        ...
        return (
            ...
            and livetime.year == now.year
            and livetime.month == now.month
            and livetime.day == now.day
        )

    def should_send_email(self):
        now = get_now_as_kst()
        return (
            not self.had_live_today()
            and now.hour >= self.shinee_usual_broadcast_finish_hour_with_buffer_in_kst
            and not is_same_date_in_kst(self.last_email_sent)
        )
