...
def get_headers(cookies, uri):
    return {
        "Cookie": create_live_cookie_string(cookies),
        "Accept": "*/*",
        "Accept-Encoding": "gzip, deflate, br",
        "Accept-Language": "ko-KR,ko;q=0.8,en-US;q=0.5,en;q=0.3",
        "Connection": "keep-alive",
        "Host": "pc-web.stream.afreecatv.com",
...
def create_live_cookie_string(cookie):
    return "; ".join(
        list(map(lambda c: str(c["name"]) + "=" + str(c["value"]), cookie))
    )
...
def get_driver(driver_type) -> WebDriver:
    ...
    if driver_type == "REMOTE":
        return webdriver.Remote(
            command_executor=CHROME, options=options
        )  # connect remote webdriver to docker standalone chrome
    else:
    ...
    broadcast_title = (
        ...
        .until(
            EC.visibility_of_element_located(
                (By.XPATH, '//*[@id="player_area"]/div[2]/div[2]/div[4]/span')
            )
        )
        .text
    )
    ...
    try:
        skip_to_low_quality_video = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="layer_high_quality"]/div/span/a')
            )
        )
...
async def scrape_with_retry(
    m3u8_url: str, filename: str, headers: dict, youtube_uploader: VideoFileUploader
):
    retrycnt = 0
...
def download_by_m3u8(
    ...
    try:
        while not finished:
            ...
            requests = [
                event for event in events if "Network.request" in event["method"]
            ]
                ...
                if res.status == 200:
                    ...
                    for seg in playlist.segments:
                        if seg.uri in uri_set_windowed:
                            ...
                            print(
                                f"Put segment: url-{url}, duration-{seg.duration} => OK"
                            )
                    ...
                    logger_config.logger.error(
                        "Received unexpected status code when requesting m3u8: {%s}",
                        res.status,
                    )
...
async def do_download(
    m3u8_url: str, filename: str, headers, youtube_uploader: VideoFileUploader
):
    """
    Start 2 tasks.
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            await asyncio.gather(
                asyncio.create_task(
                    enqueue_ts_urls(m3u8_url, headers, session, stream_queue)
                ),
                asyncio.create_task(
                    download_segments(
                        filename, stream_queue, session, headers, youtube_uploader
                    )
                ),
            )
        except KeyboardInterrupt as exc:
    ...
    while not G_QUIT:
        try:
            if os.path.getsize(os.path.realpath(file.name)) > MAX_SINGLE_FILE_SIZE:
                (old_abs_path, file) = get_next_file_and_close_current(
                    file, youtube_uploader
                )
                # Upload asynchronously
                asyncio.run_coroutine_threadsafe(
                    youtube_uploader.upload_and_delete_file_async(old_abs_path), loop
                )
